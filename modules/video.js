 ...
 * 
 * Pipeline:
 * 1. Select & trim background clips  →  clip_0.mp4, clip_1.mp4 ...
 * 2. Concatenate clips to match audio duration
 * 3. Scale to 1080x1920 (vertical / 9:16)
 * 4. Overlay color grade (brand dark aesthetic)
 * 5. Add logo watermark
 * 6. Burn subtitles (ASS)
 ...
 * @param {object} opts
 * @param {object} opts.script         - Script object with subtitleLines etc.
 * @param {string} opts.audioPath      - Path to voiceover MP3
 * @param {number} opts.audioDuration  - Duration of voiceover in seconds
...
function selectBackgroundClips(targetDuration) {
  ...
  // If no real clips exist yet, we'll generate a branded placeholder
  if (!fs.existsSync(videosDir) || fs.readdirSync(videosDir).filter(f => f.endsWith('.mp4')).length === 0) {
    logger.warn('No background clips found — using generated placeholder');
    return [];
  }
  ...
  // Shuffle and pick enough clips to fill duration
  ...
  // If still not enough, loop the first clip
...
async function prepareBackground(clips, targetDuration, outputPath, tempDir) {
  if (clips.length === 0) {
    // No clips — generate a dark branded background
  ...
  // Multiple clips: build concat list
  ...
  const listContent = normalized.map((p) => `file '${p}'`).join('\n');
...
/** ─── Branded Placeholder Background ────────────────────────── */
...
async function generateBrandedBackground(duration, outputPath) {
  ...
  // Deep black (#060400) background with animated gradient overlay
  // Uses FFmpeg lavfi for pure programmatic background
  const filter = [
    // Base dark background — brand color #060400
    `color=c=0x060400:size=1080x1920:rate=30[base]`,
    // Animated amber radial glow (pulsing)
    `color=c=0xC8720A@0.15:size=1080x1920:rate=30[amber]`,
    // Blend
    `[base][amber]blend=all_mode=overlay:all_opacity=0.3[bg]`,
    // Add grain for cinematic feel
    `[bg]noise=alls=8:allf=t[out]`,
  ].join(';');
...
/** ─── Visual Effects: Grade + Logo + Subtitles ─────────────── */
...
async function applyVisualEffects(inputPath, subtitlePath, outputPath) {
  ...
  // Escape subtitle path for FFmpeg (Windows/Unix compatible)
  ...
  if (hasLogo) {
    filterComplex = [
      // Color grade: darken slightly, boost contrast, add cinematic warmth
...
async function mixAndRender({ videoPath, voicePath, addMusic, duration, outputPath }) {
  ...
  if (hasMusic) {
    ...
    await runFFmpeg([
      ...
      '-t', String(duration),
      '-filter_complex', [
        // Voice: full volume, normalize
        '[1:a]loudnorm=I=-14:TP=-1:LRA=9,volume=1.0[voice]',
        // Music: loop, trim, reduce to background level
